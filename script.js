...
function renderStaff() {
    ...
    svg.addEventListener('click', (e) => {
        ...
        // If clicking on the far right (Flat Zone)
        if (svgX > 115) {
            const sharpOffset = offset - 1;
            const sharpNotes = instrumentNotes.filter(n => offMap[n] === sharpOffset && n.includes('#'));
            if (sharpNotes.length > 0) {
                targetNote = sharpNotes[0];
            }
        }
        
        let matchingNotes = instrumentNotes.filter(n => offMap[n] === offset);
        if (!targetNote && matchingNotes.length > 0) {
            // If clicking on the left side (Sharp Zone)
            if (svgX < 92) {
                targetNote = matchingNotes.find(n => n.includes('#'));
            }
            if (!targetNote) {
                // Otherwise, default to natural
                targetNote = matchingNotes.find(n => !n.includes('#')) || matchingNotes[0];
            }
        }

        // Fallback: If clicked offset is completely empty (no natural note exists here) 
        // e.g. clicking the Ab line (which is vertically higher), fetch the sharp.
        if (!targetNote && matchingNotes.length === 0) {
            const sharpOffset = offset - 1;
            const sharpNotes = instrumentNotes.filter(n => offMap[n] === sharpOffset && n.includes('#'));
            if (sharpNotes.length > 0) {
